...
export default function Header({ weather, coords, currentPage, setCurrentPage }) {
  ...
  return (
    <header className="h-16 border-b border-slate-800/80 bg-slate-950/60 backdrop-blur-md px-4 sm:px-6 flex items-center justify-between z-50 text-white select-none">
      {/* Location Branding */}
      ...
      {/* Navigation Switcher */}
      <nav className="flex items-center gap-1 sm:gap-2 bg-slate-900/80 p-1 rounded-2xl border border-slate-800 shadow-inner">
        <button 
          onClick={() => setCurrentPage('home')}
          className={`px-3 sm:px-4 py-1.5 rounded-xl text-xs font-medium transition ${
            currentPage === 'home' 
        ...
        <button 
          ...
          className={`px-3 sm:px-4 py-1.5 rounded-xl text-xs font-medium flex items-center gap-1.5 transition ${
            currentPage === 'copilot' 
