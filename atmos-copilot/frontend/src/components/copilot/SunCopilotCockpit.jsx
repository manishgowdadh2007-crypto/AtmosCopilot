...
import { 
  Volume2, VolumeX, Copy, Check, Sparkles, Navigation, 
  Droplets, Wind, CloudRain, Sun, Activity, ShieldAlert
} from 'lucide-react';
...
export default function SunCopilotCockpit({ 
  ...
  const [voiceProfile, setVoiceProfile] = useState('indian-female');
  ...
  // Web Speech API: Voice Synthesis Engine
  const speakText = (text) => {
    ...
    if (voiceProfile === 'indian-female') {
      utterance.voice = voices.find(v => v.lang.includes('en-IN') && (v.name.includes('Heera') || v.name.includes('Neerja') || v.name.includes('Female') || v.name.includes('Google हिन्दी'))) || voices.find(v => v.lang.includes('en-IN'));
      utterance.pitch = 1.1;
    } else if (voiceProfile === 'indian-male') {
      utterance.voice = voices.find(v => v.lang.includes('en-IN') && (v.name.includes('Ravi') || v.name.includes('Male') || !v.name.includes('Female'))) || voices.find(v => v.lang.includes('en-IN'));
      utterance.pitch = 0.95;
    } else {
      utterance.voice = voices.find(v => v.lang.includes('en-US') && (v.name.includes('Natural') || v.name.includes('Samantha') || v.name.includes('Google US')));
      utterance.pitch = 1.0;
    }
  ...
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col h-full overflow-hidden text-slate-100 select-none pb-2">
      ...
      {/* 1. Header: Avatar & Audio Controls */}
      <div className="flex flex-col items-center justify-center pt-2 pb-2 relative flex-shrink-0">
        ...
        {/* Mute & Voice Profile Switcher */}
        <div className="flex items-center gap-3 mt-2 bg-slate-900/80 border border-slate-800/90 rounded-full px-3 py-1 backdrop-blur-md">
          <button 
            onClick={() => {
          ...
          <select 
            ...
            className="bg-transparent text-[11px] font-mono text-amber-400/90 outline-none cursor-pointer"
          >
            <option value="indian-female" className="bg-slate-900 text-white">Indian English (Female)</option>
            <option value="indian-male" className="bg-slate-900 text-white">Indian English (Male)</option>
            <option value="american" className="bg-slate-900 text-white">American English</option>
          </select>
        </div>
      </div>
          ...
          {PERSONAS.map((p) => {
            ...
            return (
              <button
                ...
                className={`p-2.5 rounded-2xl border text-left transition-all duration-300 hover:-translate-y-0.5 active:scale-95 flex items-center justify-between ${
                  isActive 
      ...
      <div className="flex-shrink-0 px-4 pt-2">
        <form onSubmit={handleFormSubmit} className="relative flex items-center">
          ...
          <button
            ...
            className="absolute right-2 px-3 py-1.5 rounded-xl bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold text-xs transition disabled:opacity-40"
          >
            Send
          </button>
        </form>
